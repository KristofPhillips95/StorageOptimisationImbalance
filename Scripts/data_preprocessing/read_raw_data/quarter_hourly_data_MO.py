...
def import_quarter_hourly_MO(config_parameters):
    ...
    for i in range(len(dates_loop)):
        ...
        file = f"C:/Users/u0137781/OneDrive - KU Leuven/data/SI_forecasting/Elia/balancing_MO/ARC_VolumeLevelPrices_{n_year}_{n_month}.csv"
        ...
        if os.path.isfile(file) == True:
            ...
            df_ARC['DateTime'] = pd.to_datetime(df_ARC['Quarter'].str[:16], format='%d/%m/%Y %H:%M')
            ...
            #
            df_ARC = df_ARC.resample('15T').mean().interpolate(limit_direction='both') # Deals with spring hour and fall hour.
