def __init__(self, nn_params=None, training_params=None, OP_params=None, data_params=None, data=None, loc=None,
             config=None):
    ...
    if (nn_params is not None) & (training_params is not None) & (OP_params is not None) & (data_params is not None):
        ...
        raise ValueError(
            "Unsupported initialization. Provide arguments nn_params,training_params,OP_params and data_dict, or loc and config")
...
def load_data(self, data=None):
    if data == None:
        ...
        labels_train, labels_val, labels_test = sf.preprocess_labels(self.training_params, price_train, price_val,
                                                                     price_test,
...
def set_training_loader(self, set_seed=True):
    ...
    self.training_loader = torch.utils.data.DataLoader(train_Dataset, batch_size=self.training_params['batch_size'],
                                                       shuffle=True)


def set_sched_calculator(self):
    self.sched_calculator_mu = nnwo.Schedule_Calculator(OP_params_dict=self.OP_params,
                                                        fw=self.training_params['framework'],
                                                        dev=self.training_params['device'])
    self.sched_calculator_mu_ID = nnwo.Schedule_Calculator(OP_params_dict=self.OP_params, fw='ID',
                                                           dev=self.training_params['device'])
...
def init_loss(self):
    ...
    if self.training_params['keep_sched_train']:
        self.train_sched_evol = [
            self.sched_calculator_mu(self.data['train'][1][0], smooth=False)[0].cpu().detach().numpy()]
...
def train(self):
    ...
    for e in range(self.training_params['epochs']):
        ...
        print(
            f"Epoch {e + 1} \t\t Training Loss: {train_loss} \t\t Train time: {train_time} \t\t Loss fct: {loss_pure}; reg loss: {loss_reg}; feas loss: {loss_feas}")
...
def save(self, loc, save_data=False, save_evols=True):
    ...
    new_dict = {
        ...
        'train_times': {'time_back': self.time_backward_per_epoch, 'time_fw': self.time_forward_per_epoch,
                        'time_loss': self.time_loss_per_epoch, 'time_val': self.time_val_per_epoch,
